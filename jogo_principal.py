...
from objetos.vagalume import RebanhoVagalumes
...
class TestGameApp:
    ...
    def renderizar(self):
        ...
        sala_atual = self.room_manager.current_room
        if sala_atual in self.vagalumes_por_sala:
            self.vagalumes_por_sala[sala_atual].draw(self.tela, self.camera, self.viewport)

        # Partículas de pickup por cima de tudo
        self.gerenciador_interacao.particulas.draw(self.tela, self.camera, self.viewport)
